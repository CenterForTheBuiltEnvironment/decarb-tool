...
Full 8760-hour runs with real parquet load data, checked for:
  1. No NaNs in any output column
...
# Buildings and scenarios chosen to represent distinct configurations and climates
INTEGRATION_CASES = [
    ("5", "eq_scenario_3"),  # Office, Port Angeles (mild): AWHP + boiler + cooling
    ("5", "eq_scenario_5"),  # Office, Port Angeles: HR-WWHP + AWHP + cooling + elec resistance
    ("1", "eq_scenario_3"),  # Hospital, Denver (cold): AWHP + boiler + cooling
]
...
def _load_building(building_id: str) -> StandardLoad:
    df = pd.read_parquet(paths.LOAD_DATA_PARQUET)
    bdf = df[df["building_id"] == building_id][
        ["timestamp", "t_out_C", "heating_W", "cooling_W"]
    ].copy()
    return StandardLoad(bdf)


def _case_key(building_id: str, scenario_id: str) -> str:
    return f"building_{building_id}_{scenario_id}"
...
def integration_results(library_session):
    ...
    cache = {}
    for building_id, scenario_id in INTEGRATION_CASES:
        load = _load_building(building_id)
        df = loads_to_site_energy(load=load, library=library_session, scenario_ids=[scenario_id])
        cache[_case_key(building_id, scenario_id)] = df
    return cache


# ---------------------------------------------------------------------------
# Invariant tests (parametrized over all cases)
# ---------------------------------------------------------------------------


@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_no_nans_in_summary_columns(building_id, scenario_id, integration_results):
    ...
    """
    df = integration_results[_case_key(building_id, scenario_id)]
    always_populated = [
        Col.T_OUT_C.value,
        Col.HEATING_W.value,
        Col.COOLING_W.value,
...
@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_all_heating_served(building_id, scenario_id, integration_results):
    df = integration_results[_case_key(building_id, scenario_id)]
    heating_served = (
...
@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_all_cooling_served(building_id, scenario_id, integration_results):
    df = integration_results[_case_key(building_id, scenario_id)]
    cooling_served = (
...
@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_energy_positivity(building_id, scenario_id, integration_results):
    df = integration_results[_case_key(building_id, scenario_id)]
    assert (
...
@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_component_electricity_sum(building_id, scenario_id, integration_results):
    df = integration_results[_case_key(building_id, scenario_id)]
    elec_sum = (
...
@pytest.mark.integration
@pytest.mark.parametrize("building_id,scenario_id", INTEGRATION_CASES)
def test_annual_totals_match_golden(building_id, scenario_id, integration_results):
    ...
    key = _case_key(building_id, scenario_id)
